      ...
      <div class="mt-8 space-y-6">
        ...
        <Section id="create" title="資料を作る">
          <Steps>
            <Step n="1" title="団体を選ぶ">
              団体名などで検索して選びます。初めての団体は <K>新規団体を追加</K> から登録できます（誰でも追加できます。ロゴ画像もここで登録できます）。似た名前の団体がすでにあると警告が出るので、同じ団体なら一覧から選んでください。登録済みの団体の情報を直すのは管理者だけです。
            </Step>
        ...
        <Section id="output" title="出力・コピー">
          <ul class="list-disc space-y-2 pl-5">
            ...
            <li>メールは <K>メールソフトで開く</K> で、宛先（団体の担当者メール）・件名・本文が入った状態でメールソフトが開きます。本文が長いとメールソフトによっては途中で切れるので、そのときは <K>コピー</K> を使ってください。</li>
            <li>コピー形式が「CSV行」の媒体（サイトのお知らせデータなど）は、欄を並び順につなげた CSV の1行が出ます。<K>CSV行をコピー</K> でデータファイルの末尾に貼り付けます（見出し行も要るときは <K>見出し付きでコピー</K>）。</li>
            <li>X は1投稿目の <K>Xで開く</K> で、文面が入った投稿画面が開きます。2投稿目以降は、投稿した1投稿目への返信として <K>コピー</K> で貼り付けます。</li>
            <li><span class="font-bold">表記チェック</span>：共通設定の「表記ルール」に当てはまる言い回し（例：「御社」）があると、欄の下に黄色で知らせます。直し方が決まっているものは <K>置き換える</K> で一括で直せます。</li>
        ...
        <Section id="admin" title="管理画面">
          ...
          <ul class="list-disc space-y-2 pl-5">
            ...
            <li><K>利用状況</K> タブで、月ごとの作成数や、セット・ランク・作成者・団体ごとの件数を確認できます。</li>
          </ul>
        </Section>
...
export function AddGuide() {
  return (
    <div class="space-y-6">
      ...
      <Section id="add-syntax" title="差し込みの書き方">
        <Table
          ...
          rows={[
            ...
            [<C>{'{{部品:署名ブロック}}'}</C>, '共通パーツ（登録した文章のまとまり）を差し込む', '（パーツの内容）'],
          ]}
        />
        <ul class="list-disc space-y-2 pl-5">
      ...
      <Section id="add-parts" title="共通パーツ（署名ブロックなど、よく使う文章）" lead="いくつものテンプレートで同じ文章を使うときは、共通パーツにしておくと、直すときに1か所で済みます。">
        <Steps>
          <Step n="1">管理画面の <K>共通パーツ</K> タブ → <K>新規追加</K>。パーツ名（例：署名ブロック）と内容を書きます。内容には <C>{'{{署名}}'}</C> <C>{'{{団体名}}'}</C> などの差し込みや、別のパーツも書けます。</Step>
          <Step n="2">テンプレート編集の <K>項目を差し込む</K> の「共通パーツ」から選ぶと、<C>{'{{部品:署名ブロック}}'}</C> が入ります。書類・告知画像の雛形（Google ドキュメント／スライド）にも同じように書けます。</Step>
          <Step n="3">パーツを直すと、そのパーツを使っているすべてのテンプレートに反映されます（既存のパーツの編集は管理者）。</Step>
        </Steps>
        <Tip tone="amber">無効にしたパーツや登録の無いパーツ名は空欄で出力されます。テンプレート編集画面と資料作成の画面に警告が出ます。</Tip>
      </Section>

      <Section id="add-csv" title="CSV 1行でコピーする媒体（サイトのお知らせデータなど）" lead="サイトのお知らせを CSV で管理している場合、1件分を CSV の1行で出力して、そのまま貼り付けられます。">
        <Steps>
          <Step n="1">テンプレート編集で媒体の「＋ 新しく作る…」を選び、欄の構成を <K>CSV 1行（サイトのお知らせデータなど）</K> にします。欄は id・title・date・category・excerpt・content・image・featured の8つで作られます（CSV の見出しと同じ名前）。見出しが違う場合は、管理画面の <K>媒体</K> で欄キーを直します（欄の並び順が CSV の列の順番です）。</Step>
          <Step n="2">既存の媒体を CSV にしたいときは、管理画面の <K>媒体</K> → <K>編集</K>（管理者）で <K>コピー形式</K> を「CSV行」にします。</Step>
          <Step n="3">テンプレートの各欄に中身を書きます。例：date は <C>{'{{締結日:YYYY-MM-DD}}'}</C>、content は HTML（<C>{"<h3 class='…'>"}</C> など）をそのまま書けます。HTML の属性は <C>'</C>（シングルクォート）で囲むと CSV がすっきりします。</Step>
        </Steps>
        <Tip>欄の中の改行は、CSV では <C>{'<br>'}</C> になります。改行を入れたくないところは続けて書いてください。英数字だけの値（id・false など）以外は <C>"</C> で囲まれます。</Tip>
      </Section>

      <Section id="add-rules" title="表記ルール（出力画面の表記チェック）">
        <p>管理画面の <K>共通設定</K> に、項目キー <C>表記ルール</C> を追加し（すでにあれば編集）、値に1行に1つずつルールを書きます。</p>
        <Table
          head={['書き方', '意味']}
          rows={[
            [<C>御社→貴団体</C>, '「御社」を見つけたら知らせ、「貴団体」への置き換えボタンを出す'],
            [<C>パートナーシップ</C>, '（矢印なし）見つけたら知らせるだけ'],
            [<C># メモ</C>, '「#」で始まる行と空行は読み飛ばす'],
          ]}
        />
        <p>出力画面で各欄の下に「表記チェック」として表示されます。テンプレートそのものも直しておくと、毎回の手直しが減ります。</p>
      </Section>
      ...
      <Section id="add-orgs" title="団体">
        <ul class="list-disc space-y-2 pl-5">
          <li><span class="font-bold">1件ずつ</span>：資料作成の <K>新規団体を追加</K>、または <K>団体マスタ</K> → <K>新規追加</K>。似た名前の団体がすでにあると警告が出ます（表記ゆれ・法人格・空白の違いは同じとみなします）。</li>
          ...
          <li>団体名が空の行、すでに登録済みの団体、同じ一覧の中で重複する団体名は登録されません（登録前の一覧で確認できます）。似た名前の団体がある行には「似た団体あり」と出ます（登録はされます）。1回500件まで。ロゴは登録後に1件ずつ設定します。</li>
        </ul>
      </Section>
    </div>
  )
